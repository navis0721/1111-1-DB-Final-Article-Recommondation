...
<?php
    if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['happy'])){
        $type = 1;
        $_SESSION['type'] = 1;
        // echo "success1";
    }
    else if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['sad'])){
        $type = 2;
        $_SESSION['type'] = 2;
        // echo "success2";
    }
    else if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['angry'])){
        $type = 3;
        $_SESSION['type'] = 3;
        // echo "success3";
    }
...
$datas = array();
if($type == 1){ //happy
    $sql = "SELECT book_id, book_name, author FROM `indicators` NATURAL JOIN `information` ORDER BY happy desc";
}else if($type == 2){  //sad
    $sql = "SELECT book_id, book_name, author FROM `indicators` NATURAL JOIN `information` ORDER BY sad desc";
}else if($type == 3){  //angry
    $sql = "SELECT book_id, book_name, author FROM `indicators` NATURAL JOIN `information` ORDER BY angry desc";
}
/*
else if($type == 4){
    $sql = "SELECT * FROM `information` WHERE `book_id`>=20 and `book_id`<30";
}
else{
    $sql = "SELECT * FROM `information` WHERE `book_id`>=20 and `book_id`<30";
}

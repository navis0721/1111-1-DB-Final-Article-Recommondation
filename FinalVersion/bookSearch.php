...
<?php
    if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['score_btn'])){
        $happy_score = 100 - $_POST['happy_score'];
        $sad_score = 100 - $_POST['sad_score'];
        $angry_score = 100 - $_POST['angry_score'];
        $sum = 100 / ($happy_score + $sad_score + $angry_score);
        $_SESSION['happy_score'] = $happy_score * $sum;
        $_SESSION['sad_score'] = $sad_score * $sum;
        $_SESSION['angry_score'] = $angry_score * $sum;
        $happy_score = $_SESSION['happy_score'];
        $sad_score = $_SESSION['sad_score'];
        $angry_score = $_SESSION['angry_score'];
    }else{
        $happy_score = $_SESSION['happy_score'];
        $sad_score = $_SESSION['sad_score'];
        $angry_score = $_SESSION['angry_score'];
...
$datas = array();
$sql = "SELECT book_id, book_name, author 
        FROM `indicators` NATURAL JOIN `information` 
        WHERE happy BETWEEN $happy_score-10 AND $happy_score+10
        AND sad BETWEEN $sad_score-10 AND $sad_score+10
        AND angry BETWEEN $angry_score-10 AND $angry_score+10";
